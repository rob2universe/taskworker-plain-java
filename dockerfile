...
RUN addgroup -S javaworker && adduser -S javaworker -G javaworker
USER javaworker:javaworker
ARG JAR_FILE=target/java-worker.jar
ARG JAR_LIB_FILE=target/lib/
# cd /usr/local/runme
WORKDIR /usr/local/runme
COPY ${JAR_FILE} app.jar
ADD ${JAR_LIB_FILE} lib/
ENTRYPOINT ["java","-cp","/usr/local/runme/lib/*", "-jar","app.jar"]
